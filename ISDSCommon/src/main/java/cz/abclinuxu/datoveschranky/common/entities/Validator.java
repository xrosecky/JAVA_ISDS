...
public class Validator {
	...
	private static List<String> enabledMetaTypes = Arrays.asList("main",
			"enclosure", "signature", "meta");
	...
	static public void assertValidMessageAnnotation(String annotation) {
		if (annotation != null && annotation.length() > 255) {
			throw new IllegalArgumentException(
					"Předmět zprávy smí obsahovat nejvýše 255 znaků.");
		}
	}

	/**
	 * Zvaliduje odesílanou zprávu proti pravidlům definovaným v dokumentaci k
	 * ISDS. Jelikož přijaté zprávy mohou tato pravidla porušovat, o validaci se
	 * stará tento kód při odesílání zprávy a ne jednotlivé gettery a settery u
	 * příslušných tříd.
	 * 
	 * Tato metoda sice může odmítnout zprávy, které by rozhrání ISDS přijalo
	 * bez problémů, ale držíme se striktně dokumentace k ISDS. Příkladem je
	 * např. druh písemnosti (meta type), který může nabývat jakékoliv hodnoty a
	 * ISDS jej přijme, ačkoliv v dokumentaci jsou povoleny pouze typy main,
	 * enclosure, signature a meta.
	 * 
	 * @param message
	 *            zpráva k validaci
	 * 
	 * @throws IllegalArgumentException
	 *             pokud zpráva tato pravidla nerespektuje
	 * 
	 */
	static public void assertValidMessageForSending(Message message) {
		...
		if (attachments == null || attachments.size() == 0) {
			throw new IllegalArgumentException(
					"Zprava musi obsahovat alespon jednu prilohu.");
		}
		...
		if (legalTitle != null) {
			...
			if (legalTitle.getLaw() != null) {
				try {
					...
					throw new IllegalArgumentException(
							"Cislo paragrafu zmocneni neni prirozene cislo");
				}
			}
			// Check year
			if (legalTitle.getYear() != null) {
				try {
					...
					throw new IllegalArgumentException(
							"Rok zmocneni neni prirozene cislo");
				}
			}
		}
		...
		if (!first.getMetaType().equals("main")) {
			throw new IllegalArgumentException(String.format(
					"Druh (metatype) prvni pisemnosti "
							+ " v prilohach musí být main, tady je %s.",
					first.getMetaType()));
		}
		for (Attachment attach : attachments) {
			if (!enabledMetaTypes.contains(attach.getMetaType())) {
				throw new IllegalArgumentException(
						String.format(
								"%s není povoleny "
										+ "druh pisemnosti (metatype). Povolen jsou: %s.",
								attach.getMetaType(),
			...
			if (attach.getMimeType() == null) {
				throw new IllegalArgumentException(
						"Příloha nemá vyplněný MIME type.");
			}
		}
	}
