...
public class GetListOfReceivedMessages extends AbstractResponseParser {
	...
	static private final String[] wanting = { "dbIDSender", "dmSender",
			"dmSenderAddress", "dbIDRecipient", "dmRecipient",
			"dmRecipientAddress", "dmID", "dmAnnotation", "dmDeliveryTime",
			"dmAcceptanceTime" };
	...
	public void endElementImpl(String elName, OutputHolder handle) {
		if (super.match("dmRecords", "dmRecord")) { // máme jeden seznam
			...
			DataBox recipient = new DataBox(recipientID, recipientName,
					recipientAdress);
			String dmAnnotation = map.get("dmAnnotation").toString();
			String messageID = map.get("dmID").toString();
			MessageEnvelope env = new MessageEnvelope(MessageType.RECEIVED,
					sender, recipient, messageID, dmAnnotation);
