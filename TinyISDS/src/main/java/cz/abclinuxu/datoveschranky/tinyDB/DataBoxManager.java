 ...
 * 
 */
public class DataBoxManager implements DataBoxMessagesService,
		DataBoxDownloadService, DataBoxServices {

	private static final int MAX_REDIRECT_COUNT = 25;
	private static final List<Integer> redirectionCodes = Arrays.asList(301,
			302);
	...
	public DataBoxUploadService getDataBoxUploadService() {
		throw new UnsupportedOperationException(
				"Sluzba DataBoxUploadService neni pristupna.");
	}
	...
	public List<MessageEnvelope> getListOfReceivedMessages(Date from, Date to,
		...
		post = post
				.replace("${DATE_FROM}", XMLUtils.toXmlDate(from).toString());
	...
	public Message downloadMessage(MessageEnvelope envelope,
		...
		if (envelope.getType() != MessageType.RECEIVED) {
			throw new UnsupportedOperationException(
					"Stahnout lze pouze prijatou zpravu");
		}
		String resource = "/DownloadReceivedMessage.xml";
		String post = Utils.readResourceAsString(this.getClass(), resource);
		post = post.replace("${ID}", envelope.getMessageID());
		DownloadReceivedMessage parser = new DownloadReceivedMessage(envelope,
				storer);
	...
	public void downloadSignedMessage(MessageEnvelope envelope, OutputStream os) {
		...
		String post = Utils.readResourceAsString(this.getClass(), resource);
		post = post.replace("${ID}", envelope.getMessageID());
		DownloadSignedReceivedMessage parser = new DownloadSignedReceivedMessage(
				os);
		this.postAndParseResponse(post, "dz", parser);
	}
	 ...
	 * Vhodné pouze pro debugovací účely, ne pro zálohování.
	 * 
	 * @param envelope
	...
	private void loginImpl(String username, String password) throws Exception {
		...
		authorization = "Basic "
				+ new String(base64.encode(userPassword.getBytes()), "UTF-8");
		...
		try {
			...
			if (!rp.getStatus().ok()) {
				String message = String.format("Pozadavek selhal chybou %s:%s",
						rp.getStatus().getStatusCode(), rp.getStatus()
								.getStatusMesssage());
			...
			throw new DataBoxException("Chyba pri konfiguraci SAX parseru.",
					pce);
	...
	private void checkHttpResponseCode(HttpsURLConnection con)
			throws IOException {
		if (!OKCodes.contains(con.getResponseCode())) {
			String message = String.format(
					"Pozadavek selhal se stavovym kodem %d %s.",
