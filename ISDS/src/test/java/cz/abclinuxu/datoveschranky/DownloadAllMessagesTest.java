...
public class DownloadAllMessagesTest {
	...
	public void downloadAllMessages(int limit) throws Exception {
		...
		DataBoxMessagesService messageService = helper.connectAsFO()
				.getDataBoxMessagesService();
		DataBoxDownloadService downloadService = helper.connectAsFO()
				.getDataBoxDownloadService();
		int offset = 1;
		int count = 0;
		while (true) {
			List<MessageEnvelope> messages = messageService
