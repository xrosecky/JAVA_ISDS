...
public class Validator {
	...
	public TimeStamp readTimeStamp(byte[] timeStamp) {
		try {
			...
			String algo = OIDToAlgorithmName
					.get(tsti.getMessageImprintAlgOID());
			...
			throw new DataBoxException("IO chyba pri cteni casoveho razitka.",
					ioe);
		}
	}
			...
			for (SignerInformation signer : signers) {
				...
				if (!signer.verify(cert, "BC")) {
					throw new DataBoxException(
							"Nemohu overit oproti certifikatu stazenou zpravu.");
				}
			}
		}
	}
	...
	private X509Certificate findCertificate(SignerId signer) {
		return this.findCertificate(signer.getIssuer(),
				signer.getSerialNumber());
	}

	private X509Certificate findCertificate(Principal issuer,
			BigInteger serNumber) {
		for (X509Certificate cert : certs) {
			if (cert.getIssuerX500Principal().getName()
					.equals(issuer.getName())
					&& cert.getSerialNumber().equals(serNumber)) {
				return cert;
			}
		}
