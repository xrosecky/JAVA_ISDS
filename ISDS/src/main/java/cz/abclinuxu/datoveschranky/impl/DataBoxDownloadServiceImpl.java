...
public class DataBoxDownloadServiceImpl implements DataBoxDownloadService {
	...
	static Logger logger = LoggerFactory
			.getLogger(DataBoxDownloadServiceImpl.class);

	public DataBoxDownloadServiceImpl(DmOperationsPortType dmOpService,
			DmVoDZPortType dataBigMessageOperationsService,
	...
	/*
	 * Stahne prijatou zpravu. Pro odeslane zpravy se muzi pouzit
	 * downloadSignedMessage - omezeni na strane ISDS..
	 */
	public Message downloadMessage(MessageEnvelope envelope,
			AttachmentStorer storer) {
		logger.info(String.format("downloadMessage: id:%s",
		...
		ErrorHandling.throwIfError("Nemohu stahnout prijatou zpravu.",
				status.value);
	...
	public void downloadSignedMessage(MessageEnvelope env, OutputStream os) {
		logger.info(String.format("downloadSignedMessage: id:%s",
				env.getMessageID()));
		...
		ErrorHandling.throwIfError("Nemohu stahnout podepsanou zpravu.",
				status.value);
		try {
	...
	public void downloadSignedBigMessage(MessageEnvelope env, OutputStream os) {
		logger.info(String.format("downloadSignedBigMessage: id:%s",
				env.getMessageID()));
		String id = env.getMessageID();
